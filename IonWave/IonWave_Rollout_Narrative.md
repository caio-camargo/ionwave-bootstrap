...
| **Strategic Direction**     | High-level decisions — positioning, pricing philosophy, capital structure, partnership selection                                 | Danilo                                            | **Interim — founder-performed.** In the mature system, the Trade's thesis and decision frameworks should encode enough direction that this function is minimal.                                                                                                                                                                                               |
| **Casting**                 | Finding and evaluating the people who will execute the Trade — operators, VPs, specialists                                       | Danilo (active conversations)                     | **Interim — founder-performed.** The system needs to encode a casting methodology (see Role Engineering, CEO as Casted Actor). Open question: what is the profile of the person who can do this meta-work?                                                                                                                                                    |
| **Capital Formation**       | VP outreach, identifying advisors and investors, managing the raise                                                              | **[UNASSIGNED]** — Danilo responsible for finding this person | **Must be encoded.** Danilo does not want to do Trade-level fundraising. The methodology: start with ~20 calls, recruit more "deals people" to the cause, propagate to ~500 calls, identify a handful of advisors/investors, some of whom take on further outreach. Advisor insights orient the business plans. How to feed insights back is not yet defined. **Risk:** If Danilo doesn't find someone to own this function, it defaults back to him by gravity or doesn't happen at all — either outcome undermines the one-shot model. |
...
| **Fulfillment & CX**        | Packing/shipping, customer support, returns                                                                                      | The executor initially, CX hire later             | **Encodable.** TUPs M20/M24 contain the playbook.                                                                                                                                                                                                                                                                                                             |
| **Systems Engineering**     | Operational integrations — API setup, data ingestion, dashboards, periodic Claude agent deployments, automated monitoring        | Caio (on-call for first Trade)                    | **Pending design.** Includes: setting up Shopify/Meta APIs, building dashboards, deploying Claude agents for periodic checks (daily data validation, parameter comparison). More requirements will surface during execution. Needs advance planning.                                                                                                          |
...
| **Calibration & Oversight** | Gate decisions, strategic adjustment, pattern recognition                                                                        | Danilo (light touch post-handoff)                 | **Interim — founder-performed.** In the mature system, gate criteria should be encoded. Founder involvement should decrease toward zero.                                                                                                                                                                                                                      |
...
Danilo, meanwhile, is not in the production loop. He's giving Caio high-level direction when strategic decisions are needed, and he's talking to people — potential operators, potential collaborators, potential advisors. Ideas are percolating. He's the bottleneck right now, and that's OK. Forcing it would be worse than letting it develop.

Danilo is a founder, and a founder is a single point of failure — there is no getting around that. His approach is to abstract, go meta, and find a way where he does less and hands off more. The north star is no communication after the handoff — the Trade unfolds from the handoff alone. That's aspirational, but it orients the work: every function Danilo currently performs by hand is a function that needs to be encoded or delegated. Caio, as cofounder, can step in on oversight and calibration if needed — he doesn't have ecommerce domain expertise but has good judgment. The system is highly documented, which mitigates both cofounders' SPOF risk to some degree.
...
**Danilo is responsible for finding this person.** That's explicit and needs to be: if he doesn't, this workstream either doesn't happen (bootstrapping becomes the only path) or defaults back to him by gravity, which is exactly what he doesn't want. The risk is real — Capital Formation is the function most likely to stall because it has no owner and the founder who would naturally fill it has ruled himself out.
...
Danilo is finding the people who will execute. Candidates exist. Conversations are happening. Nobody is locked in, and this is deliberately unhurried.

What he's looking for is shaped by two frameworks. The **modularity principle** says it's not necessarily one person who does everything — it could be a solo operator, a small team, or a phased approach. The **role engineering** framework says the ontological frame of the role matters as much as the person: what someone IS determines what information they can integrate. Casting an "operator who receives a complete system" is a different frame than hiring a "startup employee." The frame shapes behavior.

One possibility that the role engineering essay surfaces: instead of Danilo finding every person himself, the Trade could be handed to a competent project-manager type who then finds and casts the operators, VPs, and specialists. This would be an intermediate step toward one-shot handoff — the PM becomes the person who reads the Trade and makes the casting decisions that Danilo currently makes by hand.
...
Meanwhile, Caio is on-call for systems engineering — but some of this work needs to happen *before* the handoff, not after. Caio is currently working on systems engineering readiness: evaluating OpenClaw as a platform-agnostic orchestration layer, and if that doesn't fit, building a solution (likely Google Apps Script) to automatically import data via API from various sources (Shopify, Meta Ads, etc.) and orchestrate integration into dashboards. This advance work is part of the rollout — the executor should arrive to a system where the data infrastructure is ready or nearly ready, not one where it needs to be built from scratch under time pressure.

During execution, Caio remains on-call. The executor thinks an automated dashboard needs adjustment — Caio can build it in hours or minutes. An inconsistency in the data is flagged — Caio moves in to fix it. A new API integration surfaces — Caio handles it. This is parenthood: the system is Caio's, and he wants to see it thrive. More requirements will surface during execution — advance planning reduces but doesn't eliminate the unforeseen.
...
- **The Producer role.** When work was split, did anyone actually hold the whole picture? Did the forcing functions (calibration, rewriting, assumption challenge) produce genuine integration? What would the Producer role need to look like for the next Trade?
...
4. **Who is the Producer?** The Actor/Director/Producer triad suggests the integrator role maps to the Producer — the person who holds the whole Trade, ensures coherence, manages the framework within which Actors and Directors operate. Is this a dedicated person? The PM-as-caster? The Operator in a solo configuration? How do they demonstrate genuine integration (not just claiming to have read everything)? What forcing functions beyond the calibration session should the system require? **Flag for Danilo.**
...
8. **What is the profile of the Producer?** The Producer holds the whole Trade, integrates everything, and casts the Actors and Directors. This might be the same person who does the meta-work of identifying operators and VPs, or it might not. Is this a PM? A recruiter? A particular kind of advisor? What does the Trade need to contain for them to do this without Danilo? And critically: what forcing functions ensure they actually know the Trade at depth?
